...
func (u *User) getTagFromUser() UserTag {
	return UserTag{u.ID, u.Username}
}
...
func (u *User) acceptFriendRequestFrom(f *User) bool {
	if !f.hasRequested(u) { // didn't send a request
		return false
	}
	u.addFriend(f)
	f.addFriend(u)
	u.removeFriendRequestFrom(f)
	return true
}

func (u *User) declineFriendRequestFrom(f *User) bool {
...
func (u *User) removeFriendRequestFrom(f *User) bool {
	r := u.FriendRequests
...
func (u *User) joinHub(h *Hub) {
	u.Hubs = append(u.Hubs, &UserHub{
		h.getTagFromHub(),
		true, // read latest will be updated when get user hub is called
	})
}
